...
function sortTable() {
    var table = $(this).parents('table').eq(0),
        rows = table.find('tr:gt(0)').toArray().sort(compareTableRows($(this).poolen()));
    this.asc = !this.asc;
    if(!this.asc) {
        rows = rows.reverse()
    }
...
// Compare table rows
function compareTableRows(poolen) {
    return function(a, b) {
        var valA = getCellValue(a, poolen), valB = getCellValue(b, poolen);
        if (!valA) { valA = 0; }
        if (!valB) { valB = 0; }
...
function getCellValue(row, poolen) {
    return $(row).children('td').eq(poolen).data("sort")
}
...
var translate = function(data) {
    ...
    $("[tkey]").each(function(poolen) {
        var strTr = data[$(this).attr('tkey')];
        $(this).html(strTr);
    });

    $("[tplaceholder]").each(function(poolen) {
        var strTr = data[$(this).attr('tplaceholder')];
	$(this).attr('placeholder', strTr)
    });

    $("[tvalue]").each(function(poolen) {
